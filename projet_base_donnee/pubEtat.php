...
$connect = pg_connect ("host=serveur-etu.polytech-lille.fr  user=cdjoumek password=postgres  dbname=gestion_publications");
     ...
     $etat=$_POST['etatArt'];
     $request1="SELECT titre_art, titre_c, annee_c, nom_ch FROM article, possede, statut_article, publie_1,conference, chercheur, ecrit  WHERE possede.p_article =article.id_art AND possede.p_statut_article=statut_article.id_sart AND publie_1.p1_conference = conference.id_c AND publie_1.p1_article= article.id_art AND ecrit.e_article= article.id_art AND ecrit.e_chercheur= chercheur.id_ch AND 
     libelle_sart='$etat'";
    ...
    $request2="SELECT titre_art, titre_l , nom_ch FROM article, possede, statut_article,publie_2,livre, chercheur, ecrit  WHERE possede.p_article =article.id_art AND possede.p_statut_article=statut_article.id_sart AND publie_2.p2_livre = livre.id_l AND publie_2.p2_article = article.id_art AND ecrit.e_article = article.id_art AND ecrit.e_chercheur = chercheur.id_ch AND libelle_sart='$etat'";
    $result2 = pg_query ($connect, $request2);
     if ($result2){ 
      ...
      $request3="SELECT titre_art, universite, annee, nom_ch FROM article, possede, statut_article, publie_3 , these, chercheur, ecrit WHERE possede.p_article =article.id_art AND possede.p_statut_article=statut_article.id_sart AND publie_3.p3_article = article.id_art AND publie_3.p3_these = these.id_t AND ecrit.e_article = article.id_art AND ecrit.e_chercheur = chercheur.id_ch AND libelle_sart='$etat'"; 
      ...
      $request4="SELECT titre_art,titre_rs, annee_rs, nom_ch FROM article, possede, statut_article,publie_4, revue_scientifique, chercheur, ecrit WHERE possede.p_article =article.id_art AND possede.p_statut_article=statut_article.id_sart AND publie_4.p4_article = article.id_art AND publie_4.p4_revue_scientifique = revue_scientifique.id_rs AND ecrit.e_article = article.id_art AND ecrit.e_chercheur = chercheur.id_ch  AND libelle_sart='$etat'"; 
